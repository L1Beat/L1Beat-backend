...
const FeesPaid = require('../src/models/feesPaid');
...
async function backfillChainFeesPaid(chainId, dryRun = false) {
  return rateLimiter.enqueue(async () => {
    try {
      console.log(`[${chainId}] Fetching 365 days of fees paid data...`);
      ...
      const response = await axios.get(
        `${metricsApiBase}/chains/${chainId}/metrics/feesPaid`,
        {
          params: {
            timeInterval: 'day',
            pageSize: 365
...
async function backfillAllChains(dryRun = false) {
  try {
    ...
    for (const chain of validChains) {
      ...
      const result = await backfillChainFeesPaid(chainId, dryRun);
      results.push(result);
      processed++;
...
console.log('=== Fees Paid Backfill Script ===');
console.log('This will fetch 365 days of fees paid data for all chains\n');
