...
const AvgGasPrice = require('../src/models/avgGasPrice');
...
async function backfillChainAvgGasPrice(chainId, dryRun = false) {
  return rateLimiter.enqueue(async () => {
    try {
      console.log(`[${chainId}] Fetching 365 days of average gas price data...`);
      ...
      const response = await axios.get(
        `${metricsApiBase}/chains/${chainId}/metrics/avgGasPrice`,
        {
          params: {
            timeInterval: 'day',
            pageSize: 365
...
async function backfillAllChains(dryRun = false) {
  try {
    ...
    for (const chain of validChains) {
      ...
      const result = await backfillChainAvgGasPrice(chainId, dryRun);
      results.push(result);
      processed++;
...
console.log('=== Average Gas Price Backfill Script ===');
console.log('This will fetch 365 days of average gas price data for all chains\n');
