...
const GasUsed = require('../src/models/gasUsed');
...
async function backfillChainGasUsed(chainId, dryRun = false) {
  return rateLimiter.enqueue(async () => {
    try {
      console.log(`[${chainId}] Fetching 365 days of gas used data...`);
      ...
      const response = await axios.get(
        `${metricsApiBase}/chains/${chainId}/metrics/gasUsed`,
        {
          params: {
            timeInterval: 'day',
            pageSize: 365
...
async function backfillAllChains(dryRun = false) {
  try {
    ...
    for (const chain of validChains) {
      ...
      const result = await backfillChainGasUsed(chainId, dryRun);
      results.push(result);
      processed++;
...
console.log('=== Gas Used Backfill Script ===');
console.log('This will fetch 365 days of gas used data for all chains\n');
