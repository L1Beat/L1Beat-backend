...
async function backfillChainTxCount(chainId, dryRun = false) {
  return rateLimiter.enqueue(async () => {
    try {
      console.log(`[${chainId}] Fetching 365 days of tx count data...`);
      ...
      const response = await axios.get(
        `${metricsApiBase}/chains/${chainId}/metrics/txCount`,
        {
          params: {
            timeInterval: 'day',
            pageSize: 365
...
async function backfillAllChains(dryRun = false) {
  try {
    ...
    for (const chain of validChains) {
      ...
      const result = await backfillChainTxCount(chainId, dryRun);
      results.push(result);
      processed++;
...
console.log('=== Daily Transaction Count Backfill Script ===');
console.log('This will fetch 365 days of tx count data for all chains\n');
