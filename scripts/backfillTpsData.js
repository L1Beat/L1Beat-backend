...
// Use 2000 requests/minute (well within the 125K CUs/minute limit at 10 CUs per request)
const rateLimiter = new RateLimiter(2000);

async function backfillChainTps(chainId, dryRun = false) {
  return rateLimiter.enqueue(async () => {
    try {
      console.log(`[${chainId}] Fetching 365 days of TPS data...`);
      ...
      const response = await axios.get(
        `${metricsApiBase}/chains/${chainId}/metrics/avgTps`,
        {
          params: {
            timeInterval: 'day',
            pageSize: 365
      ...
      console.log(`[${chainId}] Found ${validTpsData.length} valid records`);

      if (dryRun) {
        return { chainId, success: true, records: validTpsData.length, dryRun: true };
      }
...
async function backfillAllChains(dryRun = false) {
  try {
    ...
    for (const chain of validChains) {
      ...
      const result = await backfillChainTps(chainId, dryRun);
      results.push(result);
      processed++;
...
console.log('=== TPS Data Backfill Script ===');
console.log('This will fetch 365 days of TPS data for all chains\n');
