...
const MaxTps = require('../src/models/maxTps');
...
async function backfillChainMaxTps(chainId, dryRun = false) {
  return rateLimiter.enqueue(async () => {
    try {
      console.log(`[${chainId}] Fetching 365 days of max TPS data...`);
      ...
      const response = await axios.get(
        `${metricsApiBase}/chains/${chainId}/metrics/maxTps`,
        {
          params: {
            timeInterval: 'day',
            pageSize: 365
...
async function backfillAllChains(dryRun = false) {
  try {
    ...
    for (const chain of validChains) {
      ...
      const result = await backfillChainMaxTps(chainId, dryRun);
      results.push(result);
      processed++;
...
console.log('=== Maximum TPS Backfill Script ===');
console.log('This will fetch 365 days of max TPS data for all chains\n');
