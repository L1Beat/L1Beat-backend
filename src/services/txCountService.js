...
class TxCountService {
  async updateTxCountData(chainId, retryCount = config.api.metrics.rateLimit.maxRetries || 3) {
      ...
      for (let attempt = 1; attempt <= retryCount; attempt++) {
        try {
          ...
          if (!response.data || !Array.isArray(response.data.results)) {
            logger.warn(`[TxCount Update] Invalid response format for chain ${chainId}`);
            continue;
          }
          ...
          if (validData.length > 0) {
            const result = await TxCount.bulkWrite(
              validData.map(item => ({
                updateOne: {
                  filter: {
                    chainId: chainId,
            ...
            logger.info(`[TxCount Update] Updated chain ${chainId}:`, {
          ...
          logger.info(`[TxCount Update] No valid data for chain ${chainId}`);
          return {
            success: true,
            chainId,
            recordsProcessed: 0,
          ...
          logger.error(`[TxCount Update] Error for chain ${chainId} (Attempt ${attempt}/${retryCount}):`, error.message);
  ...
  async updateAllChainsTxCount() {
    try {
      logger.info('[TxCount] Starting update for all chains');
      ...
      for (const chain of chains) {
        ...
        if (!chainId || !/^\d+$/.test(String(chainId))) {
          logger.warn(`[TxCount] Skipping chain with invalid ID:`, chain.name);
          continue;
        }

        const result = await this.updateTxCountData(String(chainId));
        results.push(result);
      }
      ...
      logger.info('[TxCount] Update completed:', {
        total: chains.length,
        successful,
        failed
      });
      ...
      logger.error('[TxCount] Error updating all chains:', error);
  ...
  async getTxCountHistory(chainId, days = 30) {
    try {
      ...
      logger.error(`[TxCount] Error fetching history for chain ${chainId}:`, error);
      throw error;
    }
  }
  ...
  async getLatestTxCount(chainId) {
    try {
      ...
      logger.error(`[TxCount] Error fetching latest for chain ${chainId}:`, error);
      throw error;
    }
  }

  async getNetworkTxCountHistory(days = 30) {
    try {
      const endTime = Math.floor(Date.now() / 1000);
      const startTime = endTime - (days * 24 * 60 * 60);
      ...
      // Group by timestamp and sum tx counts
      const groupedData = {};
      data.forEach(record => {
      ...
      logger.error('[TxCount] Error fetching network history:', error);
      throw error;
    }
  }

  async getNetworkLatestTxCount() {
    try {
      ...
      const records = await TxCount.find({
        timestamp: latestTimestamp
      }).lean();

      const totalTxCount = records.reduce((sum, record) => sum + record.value, 0);
      ...
      logger.error('[TxCount] Error fetching network latest:', error);
      throw error;
    }
  }
}

module.exports = new TxCountService();
